...
pub type Size2D<T> = TypedSize2D<T, Untyped>;
...
pub fn size<T, U>(w: T, h: T) -> TypedSize2D<T, U> { TypedSize2D::new(w, h) }

impl<T: Clone, U> TypedSize2D<T, U> {
    pub fn from_typed(width: Length<T, U>, height: Length<T, U>) -> TypedSize2D<T, U> {
...
impl<Unit, T: Clone> TypedSize2D<T, Unit> {
    pub fn typed(width: T, height: T) -> TypedSize2D<T, Unit> {
