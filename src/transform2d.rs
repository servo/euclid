...
impl<T: Copy, Src, Dst> From<[T; 6]> for TypedTransform2D<T, Src, Dst> {
    fn from(array: [T; 6]) -> Self {
        Self::from_row_major_array(array)
    }
}

impl<T: Copy, Src, Dst> Into<[[T; 2]; 3]> for TypedTransform2D<T, Src, Dst> {
    fn into(self) -> [[T; 2]; 3] {
        self.to_row_arrays()
    }
}

impl<T: Copy, Src, Dst> From<[[T; 2]; 3]> for TypedTransform2D<T, Src, Dst> {
    fn from(array: [[T; 2]; 3]) -> Self {
