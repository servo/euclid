...
impl<T: Copy, U> Clone for TypedPoint2D<T, U> {
    fn clone(&self) -> TypedPoint2D<T, U> { *self }
}
...
impl<T: Copy, U> Clone for TypedPoint4D<T, U> {
    fn clone(&self) -> TypedPoint4D<T, U> { *self }
}
