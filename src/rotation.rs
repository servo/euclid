         ...
         One + Zero
{
    ...
    pub fn inverse(&self) -> TypedRotation2D<T, Dst, Src> {
        TypedRotation2D::new(-self.get_angle())
    }
    ...
    pub fn transform_point(&self, point: &TypedPoint2D<T, Src>) -> TypedPoint2D<T, Dst> {
        ...
        point2(
            ...
            point.y * Float::cos(a) + point.x * Float::sin(a),
        )
    }
...
define_matrix! {
    ...
    /// When in doubt, use `normalized_quaternion` instead of `quaternion` to create
...
impl<T, Src, Dst> TypedRotation3D<T, Src, Dst> {
    ...
    /// the real part.
    ///
    /// The resulting quaternion is not necessarily normalized. See `normalized_quaternion`.
    #[inline]
         ...
         One + Zero
{
    ...
    #[inline]
    pub fn normalized_quaternion(i: T, j: T, k: T, r: T) -> Self {
    ...
    pub fn slerp(&self, other: &Self, t: T) -> Self {
        ...
        r1.mul(Float::cos(theta)).add(r3.mul(Float::sin(theta)))
    }
    ...
    ///
    /// The input point must be use the unit Src, and the returned point has the unit Dst.
    pub fn transform_point3d(&self, point: &TypedPoint3D<T, Src>) -> TypedPoint3D<T, Dst> {
        debug_assert!(self.is_normalized());
    ...
    /// The input point must be use the unit Src, and the returned point has the unit Dst.
    #[inline]
    pub fn transform_point2d(&self, point: &TypedPoint2D<T, Src>) -> TypedPoint2D<T, Dst> {
    ...
    #[inline]
    pub fn transform_vector3d(&self, vector: &TypedVector3D<T, Src>) -> TypedVector3D<T, Dst> {
    ...
    /// The input vector must be use the unit Src, and the returned point has the unit Dst.
    #[inline]
    pub fn transform_vector2d(&self, vector: &TypedVector2D<T, Src>) -> TypedVector2D<T, Dst> {
...
fn simple_rotation_2d() {
    ...
    let r90 = Rotation2D::new(Radians::new(FRAC_PI_2));
...
fn simple_rotation_3d_in_2d() {
    ...
    assert!(ri.transform_point2d(&point2(1.0, 2.0)).approx_eq(&point2(1.0, 2.0)));
    assert!(r90.transform_point2d(&point2(1.0, 2.0)).approx_eq(&point2(-2.0, 1.0)));
    assert!(rm90.transform_point2d(&point2(1.0, 2.0)).approx_eq(&point2(2.0, -1.0)));
    assert!(r180.transform_point2d(&point2(1.0, 2.0)).approx_eq(&point2(-1.0, -2.0)));

    assert!(
        r90.inverse().inverse().transform_point2d(&point2(1.0, 2.0)).approx_eq(
...
fn slerp() {
    ...
    assert!(q1.slerp(&q2, 1.).approx_eq(&q2));
}
